...
theta1=(-100*pi)/180:0.1:pi/2;    %from -100 to 90 degrees with an increment size of pi/18
theta2=0:0.1:pi;                  %from 0 to 180 degrees with an increment size of pi/18
theta3=-pi+((10*pi)/180):0.1:0;     %from 0 to 170 degrees with an increment size of pi/5
theta4=-pi:0.1:0;                  %from -180 to 0 degrees with an increment size of pi/3



% [THETA1, THETA2] = meshgrid(theta1, theta2); % generate a grid of theta1 and theta2 values

[q1,q2,q3,q4] = ndgrid(theta1, theta2, theta3, theta4);

% X = l1 * cos(THETA1) + l2 * cos(THETA1 + THETA2); % compute x coordinates
% Y = l1 * sin(THETA1) + l2 * sin(THETA1 + THETA2); % compute y coordinates

X = cos(q1)*(a4*cos(q2 + q3) + a3*cos(q2) - d5*sin(q2 + q3 + q4));
Y = sin(q1)*(a4*cos(q2 + q3) + a3*cos(q2) - d5*sin(q2 + q3 + q4));
Z = d1 + a4*sin(q2 + q3) + a3*sin(q2) + d5*cos(q2 + q3 + q4);
